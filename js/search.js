 ...
 /* the dataset comes from the boot loader at the bottom of
    ...
    function normalizeApiTrip(apiTrip) {
        ...
        return {
            ...
            from: apiTrip.from || '',
            to: apiTrip.to || '',
            pickupStations: Array.isArray(apiTrip.pickup_stations) ? apiTrip.pickup_stations.slice() : [],
            dropoffStations: Array.isArray(apiTrip.dropoff_stations) ? apiTrip.dropoff_stations.slice() : [],
    ...
    function rememberTrip(t) {
        ...
        try {
            window.sessionStorage.setItem('etTransportSelectedTrip', JSON.stringify({ v: 1, trip: {
                ...
                from: t.from,
                to: t.to,
                pickupStations: (Array.isArray(t.pickupStations) ? t.pickupStations : (Array.isArray(t.pickup_stations) ? t.pickup_stations : [])).slice(),
                dropoffStations: (Array.isArray(t.dropoffStations) ? t.dropoffStations : (Array.isArray(t.dropoff_stations) ? t.dropoff_stations : [])).slice(),
                depart: t.depart,
                arrive: t.arrive || '',
                minutes: t.minutes,
                price: t.price,
